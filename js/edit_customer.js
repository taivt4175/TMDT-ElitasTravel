function check_input_data(data) {
    ...
    // Kiểm tra số lượng ecoin
    if (!Number.isInteger(Number(data.sl_ecoin)) || Number(data.sl_ecoin) < 0) {
        return "Số lượng ecoin phải là số nguyên lớn hơn hoặc bằng 0.";
    }
...
function editCustomer(id) {
    ...
    if (row) {
        ...
        data.sl_ecoin = cells[6].textContent;
        ...
        if (validationResult === "Dữ liệu hợp lệ") {
            ...
            xhr.open('POST', '../pscript/edit_customer.php', true);
